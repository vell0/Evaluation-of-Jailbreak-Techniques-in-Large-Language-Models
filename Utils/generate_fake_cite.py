...
def generate_citations_by_topic(args, llm, llm_judge, template_name, output_path, topic, cata="none"):
    ...
    reference_type_list = ["key", "paper", "news", "social_media", "wikipedia", "book", "github"]
        ...
        if json_context is not None:
            ...
            for rf_type in reference_type_list:
                try:
                    ...
                    if len(json_context[rf_type]) < 20:
                        flag = False
                        break
...
def main(args):
    ...
    if args.judge_harmful:
        ...
        llm_judge = load_model(judge_model_name, judge_model_path, judge_model_type, args.judge_device)
    else:
        llm_judge = None
    


    llm = LLM(args.model_name, model_path, device_map=args.device)
